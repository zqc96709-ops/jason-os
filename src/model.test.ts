...
describe('Jason OS computed views', () => {
  ...
  it('uses existing records for timeline rather than a separate entity', () => { expect(timeline([record('tasks', { updatedAt: '2026-08-09' }), record('knowledge'), record('decisions', { updatedAt: '2026-08-10' })]).map((item) => item.entity)).toEqual(['decisions', 'tasks']) })
