...
import { api, type AiProviderId, type BackupInfo, type CaptureProviderConfig, type ChatMessage, type HackStartConfig } from './api'
import {
...
function App() {
  ...
  const saveCaptureProvider = async (apiKey: string) => {
    try { setCaptureConfig(await api.configureCaptureProvider('redfox', apiKey)); showNotice('RedFoxHub 采集凭据已保存。首次采集时会验证真实连通性。') }
...
function ExternalIntelligenceView({ records, items, onOpen, onCreate, onSave, onRefresh, onAi, onNotice }: { records: RecordData[]; items: ExternalItem[]; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void>; onRefresh: () => Promise<void>; onAi: (prompt: string) => void; onNotice: (text: string, tone?: Notice['tone']) => void }) {
  ...
  const syncSource = useCallback(async (source: RecordData) => { const url = String(source.url || ''); if (!url) { onNotice('当前版本只对带公开链接的情报源执行同步；关键词和账号接口将在确认对应 Provider endpoint 后启用。', 'danger'); return } try { await api.captureLink(url); await api.save('externalSources', { ...source, lastPolledAt: new Date().toISOString(), status: 'active' }); await onRefresh(); onNotice('情报源已同步，内容已进入快速采集和外部数据层。') } catch (error) { onNotice(`同步失败：${String(error)}`, 'danger') } }, [onNotice, onRefresh])
...
function SettingsView({ config, captureConfig, onSaveAiProvider, onSaveCaptureProvider, onExport, onBackup, backups, onRestoreBackup, onRestoreRecord }: { config: HackStartConfig | null; captureConfig: CaptureProviderConfig | null; onSaveAiProvider: (provider: AiProviderId, key: string, model: string) => void; onSaveCaptureProvider: (key: string) => void; onExport: (format: 'json' | 'markdown' | 'csv') => void; onBackup: () => void; backups: BackupInfo[]; onRestoreBackup: (path: string) => void; onRestoreRecord: (id: string) => Promise<void> }) {
  ...
  const [key, setKey] = useState(''); const [redfoxKey, setRedfoxKey] = useState(''); const [model, setModel] = useState(selectedProvider?.model || 'gpt-5.5'); const [archived, setArchived] = useState<RecordData[]>([])
