...
import { api, type AiProviderId, type BackupInfo, type CaptureProviderConfig, type CaptureProviderId, type ChatMessage, type HackStartConfig } from './api'
import {
...
type View = 'command' | 'today' | 'tasks' | 'time' | 'projects' | 'outcomes' | 'finance' | 'externalIntelligence' | 'knowledge' | 'reviews' | 'insights' | 'principles' | 'mentalModels' | 'decisions' | 'events' | 'people' | 'timeline' | 'aiNews' | 'settings'
...
const defaultStatus = (entity: Entity) => ({ tasks: 'todo', goals: 'active', projects: 'active', hypotheses: 'untested', experiments: 'planned', decisions: 'pending', inbox: 'unprocessed', results: 'PLANNED', financialAccounts: 'ACTIVE', financialCategories: 'ACTIVE', financialTransactions: 'POSTED' } as Partial<Record<Entity, string>>)[entity] || 'active'
...
function App() {
  ...
  return <div className={`app-shell ${effectiveSidebarOpen ? "" : "sidebar-collapsed"} ${sidebarPeek ? "sidebar-peek" : ""} ${aiOpen ? "ai-open" : ""} ${aiResizing ? "ai-resizing" : ""}`} style={{ "--ai-width": `${aiWidth}px` } as React.CSSProperties} onPointerMove={handleShellPointer}>
    ...
    <main className="main-content">
      <div className="page-heading"><div><p className="eyebrow">JASON OS · PERSONAL OPERATING SYSTEM</p><h1>{pageTitle}</h1></div>{!['command', 'aiNews', 'timeline', 'externalIntelligence', 'outcomes', 'finance'].includes(view) && <button className="button primary" onClick={() => openCreate(viewEntity(view))}>＋ 新建</button>}</div>
      ...
      {view === 'mentalModels' && <MentalModelsView records={records} onOpen={openRecord} onCreate={openCreate} decisionModelIds={decisionModelIds} onAddModel={addModelToDecision} onRemoveModel={removeModelFromDecision} />}
...
const viewEntity = (view: View): Entity => ({ command: 'inbox', today: 'tasks', tasks: 'tasks', time: 'timeLogs', projects: 'projects', outcomes: 'results', finance: 'financialTransactions', externalIntelligence: 'signals', knowledge: 'knowledge', reviews: 'reviews', insights: 'insights', principles: 'principles', mentalModels: 'mentalModels', decisions: 'decisions', events: 'events', people: 'people', timeline: 'events', aiNews: 'inbox', settings: 'inbox' }[view] as Entity)
...
function DecisionWorkspace({ analysis, question, onOpen, onCreate, decisionModelIds, onAddModel }: { analysis: DecisionAnalysis; question: string; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; decisionModelIds: string[]; onAddModel: (id: string) => void }) {
  ...
  return <div className="decision-workspace">
    <section className="work-panel decision-classification"><header><div><span>DECISION ENGINE</span><h3>决策分析</h3></div><button className="button primary" onClick={() => onCreate('decisions', { date: today(), status: 'pending', title: question.slice(0, 60), problem: question, confidence, decisionLevel: classification.impact === 'high' ? 'STRATEGIC' : 'MATERIAL', mentalModelIds: modelIds })}>＋ 生成决策记录</button></header>
    ...
    <section className="work-panel decision-recommendation"><header><h4>Recommendation（建议，不等于决定）</h4><b>{confidence}% 信心</b></header><p>{recommendation}</p><small>最终决策由 CEO 做出；AI 只提供建议与验证路径。</small></section>
...
function MentalModelsView({ records, onOpen, onCreate, decisionModelIds, onAddModel, onRemoveModel }: { records: RecordData[]; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; decisionModelIds: string[]; onAddModel: (id: string) => void; onRemoveModel: (id: string) => void }) {
  ...
  const analysis: DecisionAnalysis | null = decisionQuestion.trim() ? runDecisionEngine(decisionQuestion, records) : null
  const decisionModels = decisionModelIds.map((id) => models.find((model) => model.id === id)).filter(Boolean) as RecordData[]
  return <div className="mental-model-engine"><section className="section-intro mental-model-hero"><span>◇</span><div><h2>CEO 决策思维模型引擎</h2><p>把思维模型从知识收藏升级为可调用、可解释、可复盘的决策认知资产。</p></div><button className="button primary" onClick={() => onCreate('mentalModels')}>＋ 新建模型</button></section>
    <section className="work-panel mental-model-toolbar"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索模型、定义、场景、标签、来源人物、触发条件…" /><div className="mental-model-filters"><div className="segmented">{mentalModelCategories.map((item) => <button key={item.id} className={category === item.id ? 'active' : ''} onClick={() => setCategory(item.id)}>{item.label}</button>)}</div><select value={sourcePerson} onChange={(event) => setSourcePerson(event.target.value)}><option value="all">全部来源人物</option>{people.map((person) => <option key={person} value={person}>{person}</option>)}</select></div></section>
    <section className="work-panel decision-model-stack"><header><div><span>DECISION MODEL STACK</span><h3>当前决策模型池</h3></div><div className="decision-stack-actions"><small>{decisionModels.length} 个模型 · 建议 3～5 个</small>{decisionModels.length > 0 && <button onClick={() => onCreate('decisions', { date: today(), status: 'pending', mentalModelIds: decisionModelIds })}>＋ 创建决策并带入模型</button>}</div></header>{decisionModels.length ? <div className="model-stack-list">{decisionModels.map((model) => <span key={model.id}>{titleFor(model)}<button onClick={() => onRemoveModel(model.id)}>×</button></span>)}</div> : <p>打开模型详情后点击“加入本次决策分析”，建立当前决策的模型组合。</p>}</section>
    {analysis && <DecisionWorkspace analysis={analysis} question={decisionQuestion} onOpen={onOpen} onCreate={onCreate} decisionModelIds={decisionModelIds} onAddModel={onAddModel} />}
    <section className="work-panel decision-question-box"><label><span>输入当前 CEO 决策问题，获取 3～5 个模型建议</span><textarea value={decisionQuestion} onChange={(event) => setDecisionQuestion(event.target.value)} placeholder="例如：是否投入50万元开发一个新产品？" /></label></section>
    <section className="model-list-section"><header><h3>模型资产 <small>{filtered.length} / {models.length}</small></h3></header>{filtered.length ? <div className="mental-model-grid">{filtered.map((model) => <article key={model.id} onClick={() => onOpen(model)}><header><span>{modelCategoryLabel(model)}</span><b>{modelSourcePerson(model)}</b></header><h3>{titleFor(model)}</h3><p>{modelDefinition(model)}</p><footer><span>触发：{modelTrigger(model)}</span><button onClick={(event) => { event.stopPropagation(); onAddModel(model.id) }}>{decisionModelIds.includes(model.id) ? '✓ 已加入' : '＋ 加入决策'}</button></footer></article>)}</div> : <GuidedEmpty icon="◇" title="还没有匹配的思维模型" text="可以调整搜索、分类或来源人物筛选。" action="创建思维模型" onAction={() => onCreate('mentalModels')} />}</section>
  </div>
}
