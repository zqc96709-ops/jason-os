...
export type CaptureProviderId = 'redfox' | 'apify'
...
export const api = {
  ...
  async getCaptureProviderConfig(): Promise<CaptureProviderConfig> { return browser() ? { providers: [{ id: 'redfox', label: 'RedFoxHub', configured: false, supportedPlatforms: ['微信公众号', '抖音', '小红书'], automaticSync: false, mediaDownload: false }, { id: 'apify', label: 'Apify', configured: false, supportedPlatforms: ['网页', '微信公众号', '抖音', '小红书', 'X', 'Instagram', 'Facebook', 'Reddit', 'TikTok', 'YouTube'], automaticSync: false, mediaDownload: false }] } : invoke('get_capture_provider_config') },
