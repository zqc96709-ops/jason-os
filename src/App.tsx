...
import './App.css'
import './toapis-theme.css'
import { buildAgentContext } from './agent/contextEngine'
import { actionsForMessage, replaceActionInMessages } from './agent/capturePlanning'
...
import { buildDailyBrief, buildFollowUpQueue, followUpRecordsWithEvidence, prepareInboxCapture, reviewInitialForResult, selectFollowUpsForDisplay, workbenchDateLabel, type DailyLane, type FollowUpItem, type FollowUpStage } from './workbenchDaily'
...
import {
  ...
  minutesToday, percent, priorityLabel, recordDate, statusLabel, timeline, titleFor, uniqueRecordsById,
...
function App() {
  ...
  const [archivedFollowUpEvidence, setArchivedFollowUpEvidence] = useState<RecordData[]>([])
  ...
  const [uiScale, setUiScale] = useState(() => normalizeUiScale(Number(localStorage.getItem('jason-os-ui-scale'))))
  ...
  const refresh = async () => {
    const { records: all, archivedFollowUpEvidence: archived, externalItems: captured } = await loadWorkbenchSnapshot(
      () => api.list('all'),
      () => api.archived(),
      () => api.listExternalItems(200),
    )
    setRecords(all); setArchivedFollowUpEvidence(archived); setExternalItems(captured)
  ...
  useEffect(() => { localStorage.setItem('jason-os-ui-scale', String(uiScale)) }, [uiScale])
  useEffect(() => {
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (modifier && !event.altKey && ['+', '=', '-', '_', '0'].includes(event.key)) {
        ...
        const command = event.key === '0' ? 'reset' : ['+', '='].includes(event.key) ? 'in' : 'out'
        setUiScale((current) => {
          const next = adjustUiScale(current, command)
          localStorage.setItem('jason-os-ui-scale', String(next))
          return next
        })
        return
      }
  ...
  const saveYoutaCapture = async (content: string) => { try { const data = prepareInboxCapture(content); await api.save('inbox', data); await refresh(); showNotice('已把你的原话保存到收集箱，Daily Brief 已同步。') } catch (error) { showNotice(String(error), 'danger') } }
  ...
  const confirmAiAction = async (actionId: string, baseChat = chat) => {
    ...
    try {
      ...
      const updated = replaceActionInMessages(baseChat, action)
  ...
  const cancelAiAction = async (actionId: string) => {
    ...
    setAiBusy(true)
    try {
      const result = await api.cancelAiAction(actionId)
      setChat((current) => replaceActionInMessages(current, result.action))
      await refresh()
    }
  ...
  const sendAi = async (preset?: string, contextOverride: Partial<AgentContext> = {}) => {
    ...
    const pending = [...chat].reverse().flatMap(actionsForMessage).find((action) => action.status === 'CONFIRM_REQUIRED')
    ...
    setAiBusy(true)
    try { const result = await api.ask(question, requestContext, nextChat); setChat([...nextChat, { role: 'assistant', content: result.answer, action: result.action, actions: result.actions }]); await refresh() }
  ...
  const saveAiProvider = async (provider: AiProviderId, apiKey: string, model: string) => {
    try { setAiConfig(await api.configureAiProvider(provider, apiKey, model)); showNotice(`${provider === 'toapis' ? 'ToAPIs' : provider === 'deepseek' ? 'DeepSeek' : provider === 'minimax' ? 'MiniMax Token Plan' : provider === 'volc-agent-plan' ? '火山引擎 Agent Plan' : 'HackStart'} 配置已保存，连通性测试通过。`) }
  ...
  const changeUiScale = (command: 'in' | 'out' | 'reset') => setUiScale((current) => {
    const next = adjustUiScale(current, command)
    localStorage.setItem('jason-os-ui-scale', String(next))
    return next
  })
  return <div className={`app-shell ${effectiveSidebarOpen ? "" : "sidebar-collapsed"} ${sidebarPeek ? "sidebar-peek" : ""} ${aiOpen ? "ai-open" : ""} ${aiResizing ? "ai-resizing" : ""}`} style={{ "--ai-width": `${Math.min(480, Math.max(400, aiWidth)) / scaleFactor}px`, "--ui-scale": scaleFactor, zoom: scaleFactor, width: `${100 / scaleFactor}%`, minHeight: `${100 / scaleFactor}vh` } as React.CSSProperties} onPointerMove={handleShellPointer}>
    <GlobalHeader
      ...
      uiScale={uiScale} onZoomIn={() => changeUiScale('in')} onZoomOut={() => changeUiScale('out')} onZoomReset={() => changeUiScale('reset')}
    />
    ...
    <main className="main-content">
      ...
      {view === 'command' && <CommandCenter records={records} archivedFollowUpEvidence={archivedFollowUpEvidence} externalItems={externalItems} onOpen={openRecord} onCreate={openCreate} onView={setView} onStartTimer={startTimer} onAi={(prompt) => { setAiDraft(prompt); setAiOpen(true) }} onCapture={saveYoutaCapture} aiConfigured={Boolean(aiConfig?.configured)} />}
...
function GlobalHeader({ query, onQuery, onSearchFocus, sidebarOpen, onToggleSidebar, onProfile, onSettings, onAiNews, aiNewsActive, onAi, onPalette, aiConfigured, uiScale, onZoomIn, onZoomOut, onZoomReset }: { query: string; onQuery: (value: string) => void; onSearchFocus: () => void; sidebarOpen: boolean; onToggleSidebar: () => void; onProfile: (section: ProfileSection) => void; onSettings: () => void; onAiNews: () => void; aiNewsActive: boolean; onAi: () => void; onPalette: () => void; aiConfigured: boolean; uiScale: number; onZoomIn: () => void; onZoomOut: () => void; onZoomReset: () => void }) {
...
function CommandCenter({ records, archivedFollowUpEvidence, externalItems, onOpen, onCreate, onView, onStartTimer, onAi, onCapture, aiConfigured }: { records: RecordData[]; archivedFollowUpEvidence: RecordData[]; externalItems: ExternalItem[]; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onView: (view: View) => void; onStartTimer: (context?: Partial<RecordData>) => void; onAi: (prompt: string) => void; onCapture: (content: string) => Promise<void>; aiConfigured: boolean }) {
  ...
  const dailyBrief = buildDailyBrief(records)
  const followUps = buildFollowUpQueue(followUpRecordsWithEvidence(records, archivedFollowUpEvidence))
  const visibleFollowUps = selectFollowUpsForDisplay(followUps)
  const laneLabels: Record<DailyLane, string> = { business: 'ToAPIs 业务', product: '产品推进', personal: '个人状态' }
  const followUpLabels: Record<FollowUpStage, string> = { action: '继续行动', result: '等待结果', review: '等待复盘' }
  const followUpCounts = (['action', 'result', 'review'] as FollowUpStage[]).map((stage) => ({ stage, count: followUps.filter((item) => item.stage === stage).length }))
  const [youtaInput, setYoutaInput] = useState('')
  const submitYoutaInput = (event: FormEvent) => { event.preventDefault(); const text = youtaInput.trim(); if (!text) return; if (aiConfigured) onAi(text); else void onCapture(text); setYoutaInput('') }
  const runFollowUp = (item: FollowUpItem) => {
    if (item.createEntity) { onCreate(item.createEntity, item.initial); return }
    const source = records.find((record) => record.id === item.sourceId)
    if (source) onOpen(source)
  }
  const personalizedPrompts = [
    '今天 ToAPIs 最值得推进的客户机会是什么？',
    '根据我最近的记录，哪些工作应该先停下来？',
    '把我刚才说的内容整理成下一步行动。',
  ]
  return <div className="command-center">
    <section className="youta-copilot"><div className="youta-copilot-heading"><span className="copilot-mark">J</span><div><p>忧太专属工作台</p><h2>告诉 Jason，你现在在想什么或准备做什么</h2><small>不用先选任务、项目或复盘。AI 会结合 ToAPIs、产品推进和你的实际记录，整理成可确认的下一步。</small></div></div><form className="youta-copilot-form" onSubmit={submitYoutaInput}><textarea value={youtaInput} onChange={(event) => setYoutaInput(event.target.value)} placeholder="例如：今天先回访两个高价值客户，再确认创作台批量流程……" rows={2} /><button className="button primary" type="submit" disabled={!youtaInput.trim()}>让 Jason 整理 →</button></form><div className="youta-copilot-prompts">{personalizedPrompts.map((prompt) => <button key={prompt} type="button" onClick={() => onAi(prompt)}>{prompt}</button>)}</div></section>
    <section className="daily-command"><header><div><p>JASON DAILY BRIEF</p><h2>今天最该推进的事</h2><span>{dailyBrief.observation}</span></div><button type="button" onClick={() => onAi('根据当前真实记录，重新判断今天最该推进的 3 件事。请分成 ToAPIs 业务、产品推进、个人状态，并说明依据。')}>让 AI 复核 →</button></header>{dailyBrief.actions.length ? <div className="daily-action-grid">{dailyBrief.actions.map((action, index) => { const record = records.find((item) => item.id === action.recordId); return <article className={`daily-action lane-${action.lane}`} key={action.recordId}><div className="daily-action-meta"><span>0{index + 1}</span><b>{laneLabels[action.lane]}</b></div><h3>{action.title}</h3><p>{action.reason}</p><div><small>下一步</small><strong>{action.nextStep}</strong></div><footer><span>来源：{action.source}</span>{record && <button type="button" onClick={() => onOpen(record)}>打开记录 →</button>}</footer></article> })}</div> : <div className="daily-empty"><strong>先告诉 Jason 一件正在发生的事</strong><p>有真实记录后，这里才会生成今日优先级，不用你手动维护另一张清单。</p></div>}<footer className="daily-guidance"><span><small>今天练习</small>{dailyBrief.practice}</span><span><small>需要纠偏</small>{dailyBrief.correction}</span></footer></section>
    <section className="follow-up-loop">
      <header className="follow-up-heading"><div><p>JASON FOLLOW-UP</p><h2>不是只提醒你做事，还要把结果收回来</h2><span>队列只来自已有任务与结果；点击先进入原记录或可编辑表单，不会自动写入。</span></div><strong>{followUps.length} 项待闭环</strong></header>
      <div className="follow-up-stages">{followUpCounts.map(({ stage, count }, index) => <Fragment key={stage}><div className={`follow-up-stage stage-${stage}`}><span>{String(index + 1).padStart(2, '0')}</span><div><small>{followUpLabels[stage]}</small><strong>{count} 项</strong></div></div>{index < followUpCounts.length - 1 && <i>→</i>}</Fragment>)}</div>
      {followUps.length ? <div className="follow-up-list">{visibleFollowUps.map((item) => <article className={`follow-up-item stage-${item.stage}`} key={item.key}><div className="follow-up-state"><span>{followUpLabels[item.stage]}</span><i /></div><div className="follow-up-copy"><h3>{item.title}</h3><p>{item.reason}</p><small>来源：{item.stage === 'action' ? '任务' : item.stage === 'result' ? '已完成任务' : '实际结果'} · {item.sourceId}</small></div><button type="button" onClick={() => runFollowUp(item)}>{item.cta}<span>→</span></button></article>)}</div> : <div className="follow-up-empty"><span>✓</span><div><strong>当前记录已经闭环</strong><p>没有到期任务、等待结果或等待复盘的明确证据。Jason 不会为了显得主动而编造提醒。</p></div></div>}
      <footer><span>行动</span><i>→</i><span>记录实际结果</span><i>→</i><span>复盘原因</span><i>→</i><span>再决定是否沉淀经验</span></footer>
    </section>
    <section className="focus-banner"><div><p>{greeting()} · {workbenchDateLabel()}</p><h2>{northStar ? titleFor(northStar) : '先定义你现在最重要的方向。'}</h2><span>{northStar ? String(northStar.why || northStar.description || '这是当前 North Star。') : '目标不是任务清单，而是帮助今天的行动有方向。'}</span></div><div className="focus-actions">{northStar ? <ProgressRing value={percent(northStar.progress)} /> : <button className="button primary" onClick={() => onCreate('goals')}>创建 North Star</button>}<button className="button ghost" onClick={() => onStartTimer(top3[0])}>▶ 开始专注</button></div></section>
